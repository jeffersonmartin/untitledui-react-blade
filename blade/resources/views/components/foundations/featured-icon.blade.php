...
$themeBase = match ($theme) {
    'light' => 'rounded-full',
    'gradient' => 'rounded-full text-fg-white before:absolute before:inset-0 before:size-full before:rounded-full before:border before:mask-b-from-0% after:absolute after:block after:rounded-full',
    'dark' => 'text-fg-white shadow-xs-skeuomorphic before:absolute before:inset-px before:border before:border-white/12 before:mask-b-from-0%',
    'modern' => 'bg-primary shadow-xs-skeuomorphic ring-1 ring-primary ring-inset',
    'modern-neue' => 'bg-primary_alt ring-1 ring-inset before:absolute before:inset-1 before:shadow-[0px_1px_2px_0px_rgba(0,0,0,0.1),0px_3px_3px_0px_rgba(0,0,0,0.09),1px_8px_5px_0px_rgba(0,0,0,0.05),2px_21px_6px_0px_rgba(0,0,0,0),0px_0px_0px_1px_rgba(0,0,0,0.08),1px_13px_5px_0px_rgba(0,0,0,0.01),0px_-2px_2px_0px_rgba(0,0,0,0.13)_inset] before:ring-1 before:ring-secondary_alt',
    'outline' => 'before:absolute before:rounded-full before:border-2 after:absolute after:rounded-full after:border-2',
};

$themeSize = match ($theme) {
    'light' => match ($size) {
        'sm' => 'size-8',
        'md' => 'size-10',
        'lg' => 'size-12',
...
<div {{ $attributes->merge(['class' => $classes, 'data-featured-icon' => true]) }}>
    {{ $slot }}
</div>
